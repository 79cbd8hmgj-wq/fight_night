...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "validate-image":
        result = validate_image(args.path, load_reference_revision(args.revision_config))
        _print_image_result(result, args.as_json)
    ...
    _print_validation_result(result, args.as_json)
    return 0 if result.valid else 1
