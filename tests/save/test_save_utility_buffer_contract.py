...
from fnr3_re.save import load_save_utility_buffer_contract
...
def test_two_controllers_build_the_same_savedata_parameter_contract() -> None:
    ...
    assert controllers["list_save_controller"].address.address_type is AddressType.ELF_VIRTUAL
    ...
    assert callbacks["save_or_autoload_payload_provider"].address.value == 0x0042CFDC
...
def test_static_contract_keeps_serializer_direction_and_ownership_unresolved() -> None:
    ...
    assert all(site.confidence is not Confidence.CONFIRMED for site in contract.controller_sites)
