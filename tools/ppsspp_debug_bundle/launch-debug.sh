...
usage() {
  cat >&2 <<'EOF'
Usage: launch-debug.sh ISO --state STATE.ppst [--port PORT]
       launch-debug.sh ISO --allow-no-state [--port PORT]
EOF
  exit 2
}
...
# PPSSPP uses RemoteISOPort for the debugger WebSocket server. The same
# configuration is copied into the memstick and explicitly appended at startup.
...
for checksum in PPSSPPSDL.sha256 PPSSPPHeadless.sha256 Xvfb.sha256; do
...
if [[ -n "$state_path" ]]; then
  ...
  installed_state="$memstick/PSP/PPSSPP_STATE/$(basename "$state_path")"
fi

"$root/stop-debug.sh" --quiet || true

# Absence of installed.txt makes this a portable PPSSPP tree. HOME is pointed
# at the bundle memstick so savedata, states, and symbols remain isolated.
export HOME="$memstick"
export XDG_CONFIG_HOME="$memstick/.config"
...
command=(
  "$root/PPSSPPSDL"
  "--appendconfig=$memstick/PSP/SYSTEM/ppsspp.ini"
  --windowed
)
if [[ -n "$installed_state" ]]; then
  command+=("--state=$installed_state")
fi
command+=("$iso_path")
printf '%q ' "${command[@]}" > "$runtime/command.txt"
printf '\n' >> "$runtime/command.txt"

(
  cd "$root"
  env LD_LIBRARY_PATH="$root/lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}" \
    "${command[@]}" >"$runtime/ppsspp.stdout.log" 2>"$runtime/ppsspp.stderr.log"
) &
ppsspp_pid=$!
...
for _attempt in $(seq 1 60); do
  if ! kill -0 "$ppsspp_pid" 2>/dev/null; then
    ...
    tail -n 100 "$runtime/ppsspp.stderr.log" >&2 || true
    exit 1
  fi
